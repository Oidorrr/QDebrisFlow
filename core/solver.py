"""
2-D LIA + O'Brien — с опциональным Numba JIT  (v1.0)
=========================================================

Numba используется если установлена и совместима с NumPy.
Если нет (например, NumPy 1.20 в QGIS 3.32) — автоматически
используется чистый NumPy. Физика и результаты идентичны.

Схема:    Local Inertial Approximation (Bates et al., 2010)
Реология: O'Brien & Julien (1985, 1988)
...
try:
    ...
    if np_ver >= np_min:
        ...
        log.info(f"Numba {_numba.__version__} активна.")
    else:
        log.warning(
            f"Numba {_numba.__version__}: NumPy {_np_ver.__version__} < "
            f"{'.'.join(map(str, np_min))} — используется NumPy."
        )
except ImportError as e:
    log.warning(f"Numba не найдена (ImportError): {e}")
except Exception as e:
    # OSError, RuntimeError и др. — Numba установлена, но неработоспособна
    log.warning(f"Numba установлена, но не инициализируется ({type(e).__name__}): {e}")
...
@dataclass
class TerrainGrid:
    """ЦМР с предвычисленной маской nodata."""
    dem:    np.ndarray
    dx:     float
    dy:     float
    nodata: float = -9999.0
    ...
    def __post_init__(self) -> None:
        ...
        # nodata → inf: поток не течёт туда (h_face ≤ 0)
...
@dataclass
class InflowSource:
    """Одна зона притока с гидрографом Q(t) [м³/с]."""
    rows:    np.ndarray
    cols:    np.ndarray
    times:   np.ndarray
    q_total: np.ndarray
...
class SimulationConfig:
    ...
    dt_max          — максимальный шаг по времени [с].
                      Реальный шаг ВСЕГДА ≤ dt_max и дополнительно
                      ограничивается условием CFL (устойчивость).
                      Не путать с output_interval.

    output_interval — интервал [с] между обновлениями прогресс-бара.
                      Не влияет на физику: за это время выполняется
                      output_interval / dt_eff шагов симуляции.
    """
    t_end:             float    = 3600.0
    dt_max:            float    = 30.0
    dt_min:            float    = 0.001
...
if _NUMBA_AVAILABLE:
    ...
    def _lia_step(h, Qx, Qy, z, mask, dx, dy, dt, h_min, n2, K_eta_8gm, tau_gm):
        ...
        for i in range(ny):
            for j in range(nxm):
                ...
                if hf != hf or hf <= h_min:
                    Qx_new[i, j] = 0.0; continue
                hf = min(hf, 0.5*(h[i, j] + h[i, j+1]))   # лимитер Almeida et al. (2012), ур. 2
                if hf <= h_min:
                    Qx_new[i, j] = 0.0; continue
                q  = Qx[i, j]
                vm = (q if q >= 0. else -q) / hf
                # Маннинг (квадр.) и вязкость (лин.) — полунеявно в знаменатель (Bates 2010)
                # τy/(γm·h) — явный импульс после шага 1 (O'Brien & Julien 1988)
                sf = n2 * vm / hf**(4./3.) + K_eta_8gm / hf**2
                d  = 1. + G * dt * sf
                if d < 1.: d = 1.
                q_star = (q - G * hf * dt * (eR - eL) / dx) / d
                yi = G * dt * tau_gm          # τy-импульс = g·Δt·τy/γm [м²/с]
        ...
        # Ограничитель оттока — двухпроходная схема (идентична NumPy-бэкенду)
        # Проход 1: коэффициент sc для каждой ячейки
        ...
        # Проход 2: к каждой грани применяем min(sc_L, sc_R) — однократно
        for i in range(ny):
            for j in range(nxm):
    ...
    def _lia_step(h, Qx, Qy, z, mask, dx, dy, dt, h_min, n2, K_eta_8gm, tau_gm):
        ...
        hf_x = np.maximum(np.nan_to_num(
            ...
            nan=0., posinf=0.), 0.)
        hf_x = np.minimum(hf_x, 0.5*(h[:,:-1]+h[:,1:]))           # лимитер Almeida et al. (2012)
        ...
        # τy — явный импульс с ограничением инверсии знака (O'Brien & Julien 1988)
...
class DebrisFlowSolver2D:
    ...
    def __init__(self, terrain, params, source, config):
        ...
        # Предвычисленные реологические константы для ядра
    ...
    def _initialise(self) -> None:
        ...
        log.info(f"[{backend}] мокрых={np.sum(self.h>self.params.h_min)}, "
                 f"V₀={self.volume_initial:.1f} м³, "
                 f"источников={len(self.source.inflow_sources)}")

        if _NUMBA_AVAILABLE and not _JIT_READY:
            log.info("Компиляция Numba JIT (~5 с)...")
            log.info(f"JIT готов за {warmup_jit(self.params.h_min):.1f} с")
            _JIT_READY = True
    ...
    def run(self) -> "SimulationResult":
        ...
        for s in self.source.inflow_sources:
            log.info(f"  '{s.name}': {s.n_cells} яч, Q_max={s.q_total.max():.1f} м³/с")
        ...
        while t_sim < t_end:
            ...
            if dt < dt_min:
                raise RuntimeError(f"CFL шаг {dt:.6f} с < dt_min={dt_min} с")
            ...
            if t_sim - t_last >= out_iv:
                ...
                log.debug(f"t={t_sim:.1f}с dt={dt:.3f}с мокрых={wet} "
                          f"V={V:.0f}м³ h_max={self.h_max.max():.2f}м")

            if ever_wet and wet == 0:
                log.info(f"Поток остановился t={t_sim:.1f} с"); break

        self.h[:]=h; self.Qx[:]=Qx; self.Qy[:]=Qy; self.t=t_sim
        log.info(f"Завершено [{backend}]: {self.n_steps} шагов, "
                 f"{t_sim:.1f} с, {time.time()-t_wall:.2f} с CPU")
